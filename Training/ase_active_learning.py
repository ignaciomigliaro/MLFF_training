...
def calculate_energies_and_std(atoms_lists, cache_file=None):
    ...
    - cache_file (str): Path to a pickle file for saving/loading energies and std_dev data. If None, no caching is done.
    ...
    if cache_file and os.path.exists(cache_file):
        ...
        return cached_data['energies'], cached_data['std_dev'], cached_data['atoms_lists']

    # Calculate energies and std_dev if no cache is found or caching is disabled
    energies = []
    for atoms in atoms_lists:
        energy_for_atoms = [atom.get_total_energy() for atom in atoms]
    ...
    if cache_file:
        data_to_save = {'energies': energies_array.tolist(), 'std_dev': std_dev, 'atoms_lists': atoms_lists}
